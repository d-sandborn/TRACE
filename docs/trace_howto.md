...
TRACE may be used to generate point estimates of anthropogenic carbon at one or more locations, under default or user-provided assumptions. Point estimates are demonstrated under **General Use**, below. More uses are demonstrated in the **Examples** pages. 
...
!!! inputs "`trace` Arguments"
    ...
    **Necessary input arguments include `output_coordinates`, `dates`, `predictor_measurements`, and `predictor_types`.**

    All other parameters are optional, with their default values listed below. 
